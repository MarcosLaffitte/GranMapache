...
g++ --version
```
###### Ith these are not installed then you need to search fr instructions on how to installing them in your respective OS. In ubuntu this can be done with an ```apt-get install```. If you already have the C and C++ compilers installed the foloow the next instructions to installed gmapache.
